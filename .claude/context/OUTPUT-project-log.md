...
**QA Round 2 → fixes (same day):** (1) **Pagination bug** — page 2 showed no products/pagination. Cause: empty-state guard used `collection.products.size` *outside* the `{% paginate %}` tag, which is empty on `?page=2`; switched to `collection.products_count` (stable total). (2) **Notch reshaped from concave arc to a STEP** per client (ref "Desktop 2"): bottom edge runs straight → short rounded riser down → lower-right shelf → bottom-right corner. Reworked the inline outline SVG (viewBox now **200×220**) and the matching objectBoundingBox clip-path; card `aspect-ratio` now **200/220** (10/11) to fit the shelf. Title moved into the shelf (padded off the stroke). Media fills the full stepped silhouette; reveal ends at `inset(0)` to fully cover. **Note:** the square-card assumption is now a 200/220 assumption — outline path + clip path + `aspect-ratio` must stay in sync if changed. Open for QA Round 3.

**QA Round 3 → fixes (same day):** Outline shape refined per client to be closer to "Desktop 5": generous outer corners (r 12→22), softer/more-integrated step (step-corner r→18, near-zero riser), card viewBox/aspect 200×220 → **200×208**, title lifted off the corner (bottom 3.5%→7%, max-width 38%→32%). **New verification technique:** rasterized the outline SVG via **headless Edge screenshot** (`msedge --headless --screenshot`, since sharp/rsvg/magick aren't available) and visually confirmed the shape before shipping — worth reusing for any future custom-shape work. Hover accent on hover confirmed good by client. Open for QA Round 4. **Reminder:** keep outline path + clip path (objectBoundingBox) + `aspect-ratio` in sync — all three encode the same 200×208 silhouette.
